...
fn main() {
    ...
    let (fuel_requirements_initial, fuel_requirements_total): (i32, i32) = fs::read_to_string(filename)
        .expect("Something went wrong reading the file")
        .lines()
        .map(|line| i32::from_str(line).expect("Parsing error"))
        .map(|module_mass| (get_fuel_requirement_for_mass(module_mass), get_complete_fuel_requirement_for_mass(module_mass)))
        .fold((0, 0), |acc: (i32, i32), next: (i32, i32)| (acc.0 + next.0, acc.1 + next.1));
